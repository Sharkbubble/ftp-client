...
#define FTP_RESPONSE_MAX_LENGTH 1024        //Maximum number of characters in a ftp server response
#define MAX_NUMBER_OF_RESPONSES 5           //Maximum number of responses against a single client command

/*
 * Data structure to hold all the responses received from a ftp server against a single client command
 */
struct ServerResponse {
	std::string responses[MAX_NUMBER_OF_RESPONSES];     //Array of string to hold the responses
    int count = 0;                                      //Count of actual responses in the array 
};
...
#define NLST_CONNECTION_CLOSE_RESPONSE "226 Closing data connection.\n"
#define FILE_SIZE_RESPONSE "213 File size (%d).\n"
#define RETR_CONNECTION_CLOSE_RESPONSE "226 Closing data connection.\n"
#define QUIT_RESPONSE "226 Closing control connection.\n"
